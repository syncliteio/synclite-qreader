...
public class JobStarter implements Job {
	...
	public void execute(JobExecutionContext context) throws JobExecutionException {
		...
		try {
			...
			propsPath = Path.of(syncLiteDeviceDir, "synclite_qreader.conf").toString();
			schedulerStatsPath = Path.of(syncLiteDeviceDir, "synclite_qreader_scheduler_statistics.db").toString();
			...
			if(currentJobPID == 0) {
				...
				if (jobRunDurationS > 0) {					
					...
					Trigger trigger = TriggerBuilder.newTrigger()
			                .withIdentity("syncLiteQReaderJobStopper-" + scheduleIndex, "syncLiteQReaderJobStopperGroup-" + scheduleIndex)
			...
			this.globalTracer.error("Failed to trigger scheduled QReader job : " + e.getMessage(), e);
		} finally {
