	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		try {
			if (request.getSession().getAttribute("synclite-device-dir") == null) {
				...
				Path confPath = Path.of(syncLiteDeviceDir, "synclite_qreader_scheduler.conf");				
				String schedulerStatsFile = Path.of(syncLiteDeviceDir, "synclite_qreader_scheduler_statistics.db").toString();
				...
				Integer numSchedules = Integer.valueOf(request.getSession().getAttribute("synclite-qreader-scheduler-num-schedules").toString());
				HashMap<String, String> properties = new HashMap<String, String>();
				properties.put("synclite-qreader-scheduler-num-schedules", request.getSession().getAttribute("synclite-qreader-scheduler-num-schedules").toString());
				
				for (int idx=1 ; idx <= numSchedules ; ++idx) {					
					String jobStartHourStr = request.getParameter("synclite-qreader-scheduler-start-hour-" + idx);
					try {
					...
					properties.put("synclite-qreader-scheduler-start-hour-" + idx, jobStartHourStr);

					String jobStartMinuteStr = request.getParameter("synclite-qreader-scheduler-start-minute-" + idx);
					try {
					...
					properties.put("synclite-qreader-scheduler-start-minute-" + idx, jobStartMinuteStr);


					String jobEndHourStr = request.getParameter("synclite-qreader-scheduler-end-hour-" + idx);
					try {
					...
					properties.put("synclite-qreader-scheduler-end-hour-" + idx, jobEndHourStr);


					String jobEndMinuteStr = request.getParameter("synclite-qreader-scheduler-end-minute-" + idx);
					try {
					...
					properties.put("synclite-qreader-scheduler-end-minute-" + idx, jobEndMinuteStr);


					String jobRunDurationStr = request.getParameter("synclite-qreader-scheduler-job-run-duration-" + idx);
					try {
					...
					properties.put("synclite-qreader-scheduler-job-run-duration-" + idx, jobRunDurationStr);

					String jobRunDurationUnit = request.getParameter("synclite-qreader-scheduler-job-run-duration-unit-" + idx);
					properties.put("synclite-qreader-scheduler-job-run-duration-unit-" + idx, jobRunDurationUnit);

					String jobRunIntervalStr = request.getParameter("synclite-qreader-scheduler-job-run-interval-" + idx);
					try {
					...
					properties.put("synclite-qreader-scheduler-job-run-interval-" + idx, jobRunIntervalStr);

					String jobRunIntervalUnit = request.getParameter("synclite-qreader-scheduler-job-run-interval-unit-" + idx);
					properties.put("synclite-qreader-scheduler-job-run-interval-unit-" + idx, jobRunIntervalUnit);

					String jobTypeStr = request.getParameter("synclite-qreader-scheduler-job-type-" + idx);
					properties.put("synclite-qreader-scheduler-job-type-" + idx, jobTypeStr);
				...
				Scheduler scheduler = (Scheduler) request.getSession().getAttribute("syncite-qreader-job-starter-scheduler-" + jobName);
				if (scheduler != null) {
					scheduler.clear();
					if (!scheduler.isShutdown()) {
						scheduler.shutdown();
					}
					request.getSession().setAttribute("syncite-qreader-job-starter-scheduler-" + jobName, null);
				}

				SchedulerFactory schedulerFactory = new StdSchedulerFactory();
				scheduler = schedulerFactory.getScheduler();
				request.getSession().setAttribute("syncite-qreader-job-starter-scheduler-" + jobName, scheduler);

				for (int idx=1 ; idx <= numSchedules ; ++idx) {					
					Integer jobRunDurationS = getDurationInSeconds(properties.get("synclite-qreader-scheduler-job-run-duration-" + idx), properties.get("synclite-qreader-scheduler-job-run-duration-unit-" + idx));
					long scheduleStartTime = getTimeInMillis(properties.get("synclite-qreader-scheduler-start-hour-" + idx), properties.get("synclite-qreader-scheduler-start-minute-" + idx));
					long scheduleEndTime = getTimeInMillis(properties.get("synclite-qreader-scheduler-end-hour-" + idx), properties.get("synclite-qreader-scheduler-end-minute-" + idx));
					Integer jobRunIntervalS= getDurationInSeconds(properties.get("synclite-qreader-scheduler-job-run-interval-" + idx), properties.get("synclite-qreader-scheduler-job-run-interval-unit-" + idx)); 
					...
					jobDataMap.put("jobType", properties.get("synclite-qreader-scheduler-job-type-" + idx));
					...
					JobDetail job = JobBuilder.newJob(JobStarter.class)
							.withIdentity("syncLiteQReaderJobStarter-" + idx, "syncLiteQReaderJobStarterGroup-" + idx)
					...
					if (jobRunIntervalS > 0) {
						globalTracer.info("Creating a periodic trigger for start time : " + properties.get("synclite-qreader-scheduler-start-hour-" + idx) + ":" + properties.get("synclite-qreader-scheduler-start-minute-" + idx) + " and end time : " + properties.get("synclite-qreader-scheduler-end-hour-" + idx) + ":" + properties.get("synclite-qreader-scheduler-end-minute-" + idx) + " with run interval : " + jobRunIntervalS + " seconds");
						trigger = TriggerBuilder.newTrigger()
								.withIdentity("syncLiteQReaderJobStarter-" + idx, "syncLiteQReaderJobStarterGroup-" + idx)
								.withSchedule(DailyTimeIntervalScheduleBuilder.dailyTimeIntervalSchedule()
										.startingDailyAt(
												TimeOfDay.hourAndMinuteOfDay(
														Integer.valueOf(properties.get("synclite-qreader-scheduler-start-hour-" + idx)), 
														Integer.valueOf(properties.get("synclite-qreader-scheduler-start-minute-" + idx))
														)
												)
										.endingDailyAt(
												TimeOfDay.hourAndMinuteOfDay(
														Integer.valueOf(properties.get("synclite-qreader-scheduler-end-hour-" + idx)), 
														Integer.valueOf(properties.get("synclite-qreader-scheduler-end-minute-" + idx))
														)
												)
						...
						globalTracer.info("Creating one time trigger for start time : " + properties.get("synclite-qreader-scheduler-start-hour-" + idx) + " and " + properties.get("synclite-qreader-scheduler-start-minute-" + idx));
