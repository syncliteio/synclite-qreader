...
<title>Configure SyncLite QReader</title>
</head>
...
<body>
	...
	<div class="main">
		<h2>Configure SyncLite QReader</h2>
		<%	
		if (errorMsg != null) {
		...
		<form method="post" action="validateDeviceDirectory">
			<table>
				<tbody>
					<tr>
						<td>Job Name</td>
						<td><input type="text" size = 30 id="job-name" name="job-name" value="<%=jobName%>" onchange="this.form.action='selectDeviceDirectory.jsp'; this.form.submit();" title="Specify SyncLite consolidator job name. Make sure that the job name specified here is same as the one specified in SyncLite Consolidator"/></td>
					</tr>
					<tr>
						<td>SyncLite Device Directory</td>
						<td><input type="text" size = 60 id="synclite-device-dir" name="synclite-device-dir" value="<%=syncLiteDeviceDir%>" title="Specify a work directory for SyncLite qreader to store SyncLite devices holding extracted data from mqtt broker."/></td>
					</tr>
				</tbody>
			</table>
			<center>
