							...
							try(ResultSet rs = stat.executeQuery("select topic_name, topic_table_name, topic_field_count, topic_create_table_sql, enable from topic_info")) {
								while (rs.next()) {
									...
									if (request.getParameter("topic-create-table-sql-" + idx) != null) {
										...
										out.println("<td><textarea id=\"topic-create-table-sql-" + idx + "\" name=\"topic-create-table-sql-" + idx + "\" rows=\"4\" cols=\"50\"" + "\" value=\"" + rs.getString("topic_create_table_sql") + "\" title=\"Specify create table sql using SQLite syntax\"/>" + rs.getString("topic_create_table_sql") + "</textarea></td>");							
									}
...
<script type="text/javascript">
	...
	function buildCreateTableSql(tableName, fieldCount) {
		const trimmedTableName = (tableName || "").trim();
		const parsedFieldCount = Number.parseInt(fieldCount, 10);

		if (!trimmedTableName || Number.isNaN(parsedFieldCount) || parsedFieldCount <= 0) {
			return "";
		}

		const columns = [];
		for (let i = 1; i <= parsedFieldCount; ++i) {
			columns.push("c" + i + " TEXT");
		}
		return "CREATE TABLE IF NOT EXISTS " + trimmedTableName + " (" + columns.join(", ") + ")";
	}

	function wireAutoSqlForRow(idx) {
		const tableInput = document.getElementById("topic-table-name-" + idx);
		const fieldCountInput = document.getElementById("topic-field-count-" + idx);
		const sqlInput = document.getElementById("topic-create-table-sql-" + idx);

		if (!tableInput || !fieldCountInput || !sqlInput || sqlInput.readOnly) {
			return;
		}

		sqlInput.dataset.autoGenerated = sqlInput.value.trim() === "" ? "true" : "false";

		const refreshSqlIfAuto = function() {
			if (sqlInput.dataset.autoGenerated !== "true") {
				return;
			}
			const generatedSql = buildCreateTableSql(tableInput.value, fieldCountInput.value);
			if (generatedSql) {
				sqlInput.value = generatedSql;
			}
		};

		tableInput.addEventListener("input", refreshSqlIfAuto);
		fieldCountInput.addEventListener("input", refreshSqlIfAuto);

		sqlInput.addEventListener("input", function() {
			sqlInput.dataset.autoGenerated = "false";
		});
	...
	if (numTablesInput) {
		const numTables = Number.parseInt(numTablesInput.value, 10);
		if (!Number.isNaN(numTables) && numTables > 0) {
			for (let idx = 0; idx < numTables; ++idx) {
				wireAutoSqlForRow(idx);
			}
		}
	}
</script>
