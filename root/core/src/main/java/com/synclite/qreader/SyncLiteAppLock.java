...
public class SyncLiteAppLock {
	...
	public final void tryLock(Path workDir) throws SyncLiteException {		
		Path lockFile = workDir.resolve("synclite_qreader.lock");
		...
		try {
			...
			throw new SyncLiteException("Failed to lock synclite qreader lock " + lockFile + ". Please stop any other qreader job running for the specified db-dir : " + workDir);
		}
	}
}
