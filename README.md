# SyncLite QReader ñ Rapid IoT Data Connector

> Part of the [SyncLite Platform](https://github.com/syncliteio/SyncLite) ñ Build Anything, Sync Anywhere.
...
- **Standard MQTT over Eclipse Paho** ñ one implementation, one protocol (MQTT v3.1 via Eclipse Paho MQTT client); works with any MQTT-compliant broker out of the box
- **One broker per job, unlimited topics** ñ each QReader job connects to a single broker URL and subscribes to all topics on it via a wildcard (`#`); run parallel jobs for multiple brokers
- **CSV payload parsing** ñ parses comma-separated (or custom-delimited) message payloads; JSON and Protobuf formats are defined in the codebase but not yet implemented
- **Schema mapping** ñ maps MQTT topic paths and CSV payload fields to destination table columns
- **QoS levels** ñ supports QoS 0, 1, and 2
- **TLS/SSL** ñ secure connections to brokers
- **Auto-reconnect** ñ survives transient broker outages with configurable retry interval
- **Web UI** ñ browser-based job configuration, live message rate gauges, and error logs
- **Edge, fog, and cloud analytics** ñ enables real-time analytics at all three tiers

## Supported Brokers

QReader uses the **Eclipse Paho MQTT v3 client** ñ there is no per-broker adapter code. Any broker that speaks standard MQTT v3.1 works automatically. The table below lists commonly tested examples:
...
| Any MQTT v3.1-compliant broker | Same code path ñ no broker-specific integration |
...
- Industrial IoT: machine sensor data ‚Üí PostgreSQL / ClickHouse analytics
- Smart building: environmental sensors ‚Üí time-series database
- Fleet tracking: GPS/telemetry ‚Üí data warehouse
- Energy monitoring: smart meter readings ‚Üí data lake
...
‚Üê Back to the [SyncLite Platform README](https://github.com/syncliteio/SyncLite/blob/main/README.md)
